<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                                ...
                                <tr v-for="product in products.data" :key="product.id"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link :href="route('admin.products.edit', product.id)"
                                              class="text-indigo-600 hover:text-indigo-900 mr-4">Редактировать
                                        </Link>
                                        <button @click.prevent="deleteProduct(product.id)"
                                                class="text-red-600 hover:text-red-900">Удалить
                                        </button>
                                    </td>
                                </tr>
                                </tbody>
        ...
        <div  v-else class="sm:flex sm:items-center sm:justify-between mb-8">
            <h1 class="text-3xl font-extrabold text-gray-900">Список товаров пуст</h1>
            <div class="mt-4 sm:mt-0">
                <Link :href="route('admin.products.create')"
                      class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                    <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
